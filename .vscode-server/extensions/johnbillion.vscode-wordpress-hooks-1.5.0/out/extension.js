...
const actions_json_1 = require("@wp-hooks/wordpress-core/hooks/actions.json");
const filters_json_1 = require("@wp-hooks/wordpress-core/hooks/filters.json");
const extensionName = 'vscode-wordpress-hooks';
function getHookCompletion(hook) {
...
function getHook(name) {
    let hooks = filters_json_1.hooks.filter((filter) => {
        var _a;
        if (filter.name === name) {
            return true;
        }
        if ((_a = filter.aliases) === null || _a === void 0 ? void 0 : _a.includes(name)) {
            return true;
        }
        return false;
    });
    if (hooks.length === 0) {
        hooks = actions_json_1.hooks.filter((action) => {
            var _a;
            if (action.name === name) {
                return true;
            }
            if ((_a = action.aliases) === null || _a === void 0 ? void 0 : _a.includes(name)) {
                return true;
            }
            return false;
        });
...
function getReturnType(tag) {
    return getTagType(tag);
}
function getContainingSymbol(symbols, position) {
...
function activate(context) {
    const hooksProvider = vscode.languages.registerCompletionItemProvider('php', {
        provideCompletionItems(document, position) {
            ...
            if (isInAction(linePrefix)) {
                return actions_json_1.hooks.map(getHookCompletion);
            }
            if (isInFilter(linePrefix)) {
                return filters_json_1.hooks.map(getHookCompletion);
            }
            return undefined;
        },
    }, "'", '"');
    const callbackProvider = vscode.languages.registerCompletionItemProvider('php', {
        provideCompletionItems(document, position) {
            ...
            const hook = getHook(((_a = declaration.groups) === null || _a === void 0 ? void 0 : _a.hook) || '');
            if (!hook) {
                return undefined;
            }
            const completions = [];
            ...
            const paramTypeLengths = [0];
            const paramNameLengths = [0];
            ...
            params.forEach((param) => {
                var _a, _b;
                const types = ((_a = param.types) === null || _a === void 0 ? void 0 : _a.join('|').padEnd(longestParamType, ' ')) || '';
                const variable = ((_b = param.variable) === null || _b === void 0 ? void 0 : _b.padEnd(longestParamName, ' ')) || '';
            ...
            if (hook.type === 'filter' || hook.type === 'filter_reference') {
                ...
                docblockLines.push(` * @return ${((_b = params[0].types) === null || _b === void 0 ? void 0 : _b.join('|')) || ''} ${params[0].content}`);
            }
            else {
            ...
            docblockLines.push(' */');
            const docBlocksEnabled = (_c = vscode.workspace.getConfiguration(extensionName).get('docBlocks.enable')) !== null && _c !== void 0 ? _c : true;
            ...
            if (vscode.window.activeTextEditor !== undefined) {
                return vscode.commands
                    .executeCommand('vscode.executeDocumentSymbolProvider', vscode.window.activeTextEditor.document.uri)
                    .then((symbols) => {
                    ...
                    if (symbols === undefined) {
                        completionItemForFunction.additionalTextEdits.push(vscode.TextEdit.insert(insertionPosition, '\n\n'));
                        if (docBlocksEnabled) {
                            completionItemForFunction.additionalTextEdits.push(vscode.TextEdit.insert(insertionPosition, `${docblockLines.join('\n')}\n`));
                        }
                        completionItemForFunction.additionalTextEdits.push(vscode.TextEdit.insert(insertionPosition, insertFunction));
                    ...
                    if (positionContext.inMethod && positionContext.symbol) {
                        ...
                        if (docBlocksEnabled) {
                            completionItemForMethod.additionalTextEdits.push(vscode.TextEdit.insert(positionContext.symbol.range.end, `${docblockLines.map((line) => `${leadingWhitespace}${line}`).join('\n')}\n`));
                        }
                        completionItemForMethod.additionalTextEdits.push(vscode.TextEdit.insert(positionContext.symbol.range.end, insertMethod));
                        ...
                        completionItemForFunction.additionalTextEdits.push(vscode.TextEdit.insert(insertionPosition, '\n\n'));
                        if (docBlocksEnabled) {
                            completionItemForFunction.additionalTextEdits.push(vscode.TextEdit.insert(insertionPosition, `${docblockLines.map((line) => `${leadingWhitespace}${line}`).join('\n')}\n`));
                        }
                        completionItemForFunction.additionalTextEdits.push(vscode.TextEdit.insert(insertionPosition, insertFunction));
                        completions.push(completionItemForFunction);
                    }
